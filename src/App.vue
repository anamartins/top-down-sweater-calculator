  ...
  <div class="wrapper">
    ...
    <section class="sample">
      ...
      <div class="form" id="form-sample">
        <number-input label="Rows:" unit=" " v-model:value="sample.rows" id="rows" />
        <number-input label="Stitches:" unit=" " v-model:value="sample.stitches" id="stitches" />
      </div>
    </section>
    ...
    <section class="measures">
      ...
      <div class="form" id="form-measures">
        <number-input label="Neckline:" v-model:value="measures.neckline" />
        <number-input label="Armhole:" v-model:value="measures.armhole" />
        <number-input label="Shoulder to shoulder:" v-model:value="measures.carrure" />
        <number-input label="Chest:" v-model:value="measures.chest" />
        <number-input label="Biceps:" v-model:value="measures.biceps" />
        <number-input label="Wrist:" v-model:value="measures.wrist" />
        <number-input label="Arm:" v-model:value="measures.arm" />
      </div>
    </section>

    <section class="results" v-if="shouldShowResults">
      <h2>Results</h2>
      <p>Cast on <span class="number-result">{{results.neckline}}</span> stitches and make a circle. Make your collar
        using stitch and knit as many rows
        as you want. Your yoke will have <span class="number-result">{{results.armholeRows}}</span> rows. It means you
        need to make
        {{results.increaseCycles}} increase cycles (1 increasing row and {{details.perRow - 1}} regular row). In your
        increasing rows, you have
        to knit one extra stitch every <span class="number-result">{{results.yokeIncrease.every2}}</span> stitches.
        You'll end with
        <span class="number-result">{{results.armholeEnd}}</span> stitches.
      </p>
      ...
      <ol>
        ...
        <li>cast on more <span class="number-result">{{results.differenceChestCarrure}}</span> stitches on the needle
          for the underarm part of the body;</li>
        ...
        <li>cast on more <span class="number-result">{{results.differenceChestCarrure}}</span> stitches on the needle
          for the underarm part of the body;</li>
        <li>knit <span class="number-result">{{Math.floor(results.body / 2)}}</span> stitches for the the back part.
          Knit the body until the end. Go to the sleeves.
          Pass to the needle the <span class="number-result">{{results.yokeSleeves}}</span> stitches from the pin. Don't
          forget to add the
          <span class="number-result">{{results.differenceChestCarrure}}</span> stitches for the underarm part of the
          sleeve.
        </li>
